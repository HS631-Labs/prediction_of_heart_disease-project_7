...
sqrt_age <- sqrt((max(heart_dataframe$Age)+1) - heart_dataframe$Age) 
sqrt_age
sqrt_age_hist <- hist(sq_age)
...
#Chunk 8: Correlation Matrix of only numeric varibales by subsetting heart_dataframe
```{r}
...
#Chunk 9: Data conversion to all numeric to obtain a complete correlation matrix and correlation plot 
```{r}
...
log_dataframe <- data.frame(sq_age, log_cholesterol,log_RBP, heart_dataframe$MaxHR, heart_dfnum$ST_Slope, heart_dfnum$Sex, heart_dfnum$ChestPainType, heart_dfnum$FastingBS,heart_dfnum$RestingECG, heart_dfnum$ExerciseAngina, heart)
str(log_dataframe)
...
heart_allnum_cor 
heart_allnum_corplot <- corrplot(heart_allnum_cor, method = "circle")

heart_logcor <- cor(log_dataframe, use = "complete.obs")
heart_logcor
heart_logcorplot <- corrplot(heart_logcor, method = "square")
