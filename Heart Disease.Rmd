...
#library(DMwR) # for smote implementation - NOT AVAILABLE FOR THIS VersionR
...
heart_allnum_cor <- cor(heart_dfnum, use = "complete.obs", method = "spearman")
heart_allnum_cor 
...
# Chunk 13.0: Removing Outliers for Model 3 
```{r}
#find outliers 
...
#Chunk 13.1:  Logistic Regression Model 3: 
```{r}
...
#Chunk 14: Comparing  model 1 & 2.1 and  2.1 & 2.2 performance - ANOVA 
```{r}
...
#chunk 14.1  compare models 2.2_reduced and 3.2 - PseudoR2
```{r}
...
# Chunk 14.2 Confusion matrix for  models 2.2_reduced and 3.2 using heart dataset with no outliers 
```{r}
...
table(heartdf_no_outlier$eval_1) #model2.2_reduced with heartdf_no_outlier
...
table(heartdf_no_outlier$eval_2)#model3.2 with heartdf_no_outlier 
```

#Chunk 14.3 ROC Curve 
```{r}
...
#For every unit change in interaction between Nature of ST Slope and Presence of Exercise Induced Angina there was a multiplication in the odds of presence of heart disease by a factor of 2.5. 
```
